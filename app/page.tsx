      ...
      <main className="flex flex-1 flex-col">
        <section className="mx-auto grid w-full max-w-[1120px] grid-cols-1 gap-10 px-[18px] py-10 min-[900px]:grid-cols-[1.05fr_0.95fr] min-[900px]:items-center min-[900px]:gap-14 min-[900px]:px-8 min-[900px]:py-16">
          <div className="flex flex-col gap-5">
            ...
            <p className="max-w-[52ch] font-sans text-copy leading-[1.65] text-body min-[900px]:text-subhead min-[900px]:leading-[1.6]">
              Find your masjid or community center, choose a campaign, and check out. Stripe emails
              the receipt. Every approved expense is published back to the campaign — with the
              receipt attached.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <Link href="#how-it-works" className={buttonClasses({ size: "lg", variant: "secondary" })}>
                How it works
              </Link>
            </div>
            ...
            <ul className="grid list-none grid-cols-1 gap-x-6 gap-y-2 min-[480px]:grid-cols-2">
              {TRUST_ITEMS.map((item) => (
                <li key={item} className="flex items-center gap-1.5 text-meta text-body">
                  <CheckIcon />
                  {item}
                </li>
              ))}
          ...
          {featured ? (
            ...
            <div className="flex min-h-[280px] flex-col justify-center gap-2 rounded-xl border border-dashed border-border bg-surface-sunken px-6 py-8">
              <p className="font-sans text-copy font-semibold text-ink">No live campaigns yet</p>
              <p className="text-meta text-body">
                When an organization publishes a campaign, it will appear here with its goal,
                phases, and posted expenses.
              </p>
            </div>
          )}
        </section>
        ...
        <section className="border-y border-hairline bg-surface-sunken">
          <dl className="mx-auto grid w-full max-w-[1120px] grid-cols-2 gap-px min-[900px]:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="flex min-w-0 flex-col gap-1 px-[18px] py-5 min-[900px]:px-8">
          ...
          <section id="how-it-works" className="flex flex-col gap-6 scroll-mt-8">
            <div className="flex flex-col gap-1.5">
              ...
              <p className="max-w-[56ch] text-meta text-body">
                Three steps for a donor, and one loop that closes: money in, work done, receipt
                posted.
              </p>
            </div>
